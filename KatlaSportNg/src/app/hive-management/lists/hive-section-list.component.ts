...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number){
    ...
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, true).subscribe(h => hiveSection.isDeleted = true)
  }

  onUndelete(hiveSectionId: number){
    var hiveSection = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, false).subscribe(h => hiveSection.isDeleted = false)
  }
}
